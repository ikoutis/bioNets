# Here we do inner product coupled with TOM along with divide by norm and apply scale free criteria
...
source('ScaleFree.R')
sft
rm(corOptionsm, corx, corxCur, corxPowers, corxPrev, data, datk, datout)
...
adj_exper11 <- innerProduct^softpower
print("Scale-free criteria is applied")

#Applying Topological Overlap matrix
TOM <- TOMsimilarity(adj_exper11)
print("The adjacency matrix is created")
...
geneTree_exper11 <- hclust(as.dist(dissTOM), method = "average")

#Plot the resulting clustering tree
dev.new()
sizeGrWindow(12,9)
plot(geneTree_exper11, xlab = "", sub = "", main = "Gene clustering on exper11 dissimilarity",
...
dynamicMods_exper11 <- cutreeDynamic(dendro = geneTree_exper11, method = "tree",
                                 minClusterSize = minModuleSize, deepSplit = FALSE)

#Returns the label of the cluster per gene
table(dynamicMods_exper11)

#Converting the numeric labels to colors
dynamicColors_exper11 <- labels2colors(dynamicMods_exper11)
table(dynamicColors_exper11)

#Plot the dendogram and the color underneath
dev.new()
sizeGrWindow(8,6)
plotDendroAndColors(geneTree_exper11, dynamicColors_exper11, "Dynamic Tree Cut",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05,
                    main = "Dendro and Cluster Color based on exper11 similarity")
...
for(i in 1:dim(clusters_label_freq_exper11)[1]){
  #Extracting the i-th label
  label <- clusters_label_freq_exper11$moduleLabels_exper11[i]
  label <- droplevels(label)
  label <- as.numeric(levels(label))
  #
  ##label <- 11
  cluster <- -1
  for(j in 1:length(moduleLabels_exper11)){
    if(moduleLabels_exper11[j] == label){
      cluster = rbind(cluster, EntrezID_NoOutlier[j])
    }
  }
  cluster <- cluster[-1]
  if(length(cluster) != clusters_label_freq_exper11$Freq[i])
  ...
  library(plyr)
  clusters_GO_exper11 = rbind.fill(clusters_GO_exper11, res)
...
caption_title <- paste("GSE", GSE, "_exper11.RData" ,sep = "")
save(moduleLabels_exper11, geneTree_exper11, clusters_GO_exper11, clusters_label_freq_exper11, file = caption_title )
#Followiing items will be saved:
#     moduleLabels_exper11           =>  numeric label of the clusters
#     geneTree_exper11               =>  denrogram of the clustering
#     clusters_GO_exper11            =>  Summary of the gene ontology annotation
#     clusters_label_freq_exper11    => table of the clusters label and their corresponding frequncies
######################################################################################################################
